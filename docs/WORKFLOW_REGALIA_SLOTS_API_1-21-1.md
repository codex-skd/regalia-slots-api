...
## Capa de compatibilidad Curios API (introducida en v0.0.0-beta.4, estable desde v1.0.0)
...
- **Renderizado de curios de terceros (desde v0.0.0-beta.15)**: `compat/curios/CuriosRendererAdapter` + reenvío en `top.theillusivec4.curios.api.client.CuriosRendererRegistry#register` conectan los renderers cliente que registran mods como Relics (clase de Curios copiada verbatim, antes huérfana: sus mapas no los leía nadie y `RegaliaSlotsApiLayer` solo consulta `RegaliaSlotsApiRendererRegistry`) a la capa de render real. El `RegaliaSlotsApiRendererRegistry.load()` que ya llama `EntityRenderersEvent.AddLayers` los materializa; no se añade otro listener. Verificado build; render en juego sin verificar.
- **Huecos conocidos** (no bloquean reconocimiento de ranura ni lectura/escritura del inventario): comportamiento *custom* por item (`ICurioItem`/`ICurio`, ~20 métodos: sonidos, glow, tooltips) no puenteado — items registrados como curio reciben comportamiento por defecto; codecs de datapack nativo de Curios (`ISlotType`/`ISlotData`/`IEntitiesData`) son stubs (Regalia sigue siendo la única fuente de verdad para tipos de ranura). La pantalla de cliente de Curios (`top.theillusivec4.curios.client.gui.CuriosScreen`) está presente únicamente como shim de carga para evitar `NoClassDefFoundError` en mods que la referencian (ej. Apothic Attributes); como `RegaliaSlotsApiScreen` no extiende este tipo, los mods que integran con la pantalla de inventario de Curios se cargan correctamente pero su integración no se adjunta realmente — hueco conocido y aceptado.

 ...
 */
public abstract class RegaliaSlotsApiDataProvider implements DataProvider {
  ...
  private final RegaliaSlotsApiItemTagsProvider itemTagsProvider;

  public RegaliaSlotsApiDataProvider(String modId, PackOutput output,
    ...
    RegaliaSlotsApiBlockTagsProvider blockTagsProvider =
        new RegaliaSlotsApiBlockTagsProvider(output, registries, modId);
    this.itemTagsProvider =
        new RegaliaSlotsApiItemTagsProvider(output, registries, blockTagsProvider.contentsGetter(),
                                   modId);
  }
  ...
  public final TagAppender<Item> tag(String slotId) {
    return this.itemTagsProvider.tag(
        TagKey.create(Registries.ITEM, RegaliaSlotsApiResources.resource(slotId)));
  }

  public final TagAppender<Item> tag(ISlotData slot) {
    return this.itemTagsProvider.tag(
        TagKey.create(Registries.ITEM, RegaliaSlotsApiResources.resource(slot.getId())));
  }

  @Nonnull
  public final String getName() {
    return "RegaliaSlotsApi for " + this.modId;
  }

  private static ISlotData createSlotData(String id) {
    return RegaliaSlotsApiServices.SLOTS.getSlotData(id);
  }

  private static IEntitiesData createEntitiesData() {
    return RegaliaSlotsApiServices.SLOTS.getEntitiesData();
  }

  private static class RegaliaSlotsApiBlockTagsProvider extends BlockTagsProvider {

    public RegaliaSlotsApiBlockTagsProvider(PackOutput output,
  ...
  private static class RegaliaSlotsApiItemTagsProvider extends BlockTagCopyingItemTagProvider {

    CompletableFuture<HolderLookup.Provider> lookupProvider;

    public RegaliaSlotsApiItemTagsProvider(PackOutput output,
                                  CompletableFuture<HolderLookup.Provider> lookupProvider,

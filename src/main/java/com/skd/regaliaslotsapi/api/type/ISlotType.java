...
/**
 * This interface provides read-only data for slot types used throughout RegaliaSlotsApi. Modders are not
...
public interface ISlotType extends Comparable<ISlotType> {
  ...
  static ISlotType get(String id) {
    return RegaliaSlotsApiSlotTypes.getSlotTypes().get(id);
  }
   ...
   * of the same nature in the RegaliaSlotsApi inventory screen. A slot type that is omitted is expected
   ...
   *
   * <p>A render toggle appears as a small button in the top-right corner of the RegaliaSlotsApi inventory
   ...
   * {@link com.skd.regaliaslotsapi.api.common.DropRule} and globally through
   * {@link com.skd.regaliaslotsapi.api.event.DropRulesEvent}.
   *
   ...
   * {@link com.skd.regaliaslotsapi.api.type.capability.ICurioItem#canEquip(SlotContext, ItemStack)}
   * and {@link com.skd.regaliaslotsapi.api.event.CurioCanEquipEvent}.
   *
   ...
   * {@link RegaliaSlotsApiSlotTypes#registerPredicate(Identifier, BiPredicate)}.
   *
  ...
  default boolean isItemValid(SlotContext slotContext, ItemStack stack) {
    return RegaliaSlotsApiSlotTypes.testPredicates(slotContext, stack, this.getValidators());
  }

  /**
   * The location of an image icon to use when specified image icons are null, missing, or invalid.
   */
  Identifier GENERIC_ICON = RegaliaSlotsApiResources.resource("textures/gui/generic_icon.png");

  /**
   * The codec used for (de)serializing slot type data.
   */
  Codec<ISlotType> CODEC = RegaliaSlotsApiServices.CODECS.slotTypeCodec();
  ...
  StreamCodec<RegistryFriendlyByteBuf, ISlotType> STREAM_CODEC =
      RegaliaSlotsApiServices.CODECS.slotTypeStreamCodec();

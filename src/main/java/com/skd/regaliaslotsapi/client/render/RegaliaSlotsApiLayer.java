...
package com.skd.regaliaslotsapi.client.render;
...
public class RegaliaSlotsApiLayer<T extends LivingEntity, M extends EntityModel<T>> extends
    RenderLayer<T, M> {

  private final RenderLayerParent<T, M> renderLayerParent;

  public RegaliaSlotsApiLayer(RenderLayerParent<T, M> renderer) {
    super(renderer);
        ...
        .ifPresent(handler -> handler.getCurios().forEach((id, stacksHandler) -> {
          ...
          for (int i = 0; i < stackHandler.getSlots(); i++) {
            ...
            if (!stack.isEmpty()) {
              ...
              RegaliaSlotsApiRendererRegistry.getRenderer(stack.getItem()).ifPresent(
                  renderer -> renderer

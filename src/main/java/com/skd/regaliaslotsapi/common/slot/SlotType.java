package com.skd.regaliaslotsapi.common.slot;
  ...
  public static class Builder {
    ...
    public void apply(ISlotData.Entry entry, HolderLookup.Provider provider) {
      ...
      entry.validators().ifPresentOrElse(list -> {
        ...
        if (!replace) {
          this.validators.add(RegaliaSlotsApiResources.resource("tag"));
        }
      });
      ...
      entry.entities().ifPresentOrElse(list -> {
        ...
        if (!replace) {
          BuiltInRegistries.ENTITY_TYPE.get(RegaliaSlotsApiTags.PLAYER_LIKE)
    ...
    public ISlotType build() {
      ...
      if (this.validators == null) {
        this.validators = Set.of(RegaliaSlotsApiResources.resource("tag"));
      }

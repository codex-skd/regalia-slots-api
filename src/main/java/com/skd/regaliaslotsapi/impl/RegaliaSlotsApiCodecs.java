...
public class RegaliaSlotsApiCodecs implements IRegaliaSlotsApiCodecs {
  ...
  public Codec<Holder<Attribute>> slotAttributeCodec() {
    return Identifier.CODEC.xmap(
        resourceLocation -> {
          if (resourceLocation.getNamespace().startsWith(RegaliaSlotsApiResources.MOD_ID)) {
          ...
          return SlotAttribute.getOrCreate(RegaliaSlotsApiSlotTypes.Preset.CURIO.id());
        },
        attributeHolder -> {
          if (attributeHolder.value() instanceof SlotAttribute slotAttribute) {
            return RegaliaSlotsApiResources.resource(slotAttribute.id());
          }
          return RegaliaSlotsApiResources.resource(RegaliaSlotsApiSlotTypes.Preset.CURIO.id());
        }
    );
  }
  ...
  public StreamCodec<RegistryFriendlyByteBuf, Holder<Attribute>> slotAttributeStreamCodec() {
    return new StreamCodec<>() {
      ...
      public Holder<Attribute> decode(@Nonnull RegistryFriendlyByteBuf buffer) {
        ...
        if (resourceLocation.getNamespace().equals(RegaliaSlotsApiConstants.MOD_ID)) {
